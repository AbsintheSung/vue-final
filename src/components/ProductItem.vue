<script setup>
import { watch } from "vue";
const props = defineProps({
  allCartInfo: {
    type: Object,
    default: () => {
      return {};
    },
  },
});
watch(
  () => props.allCartInfo,
  (newVal) => {
    console.log("Updated allCartInfo:", newVal);
  },
  { immediate: true }
);
</script>
<template>
  <div class="border p-4 mb-4">
    <!-- <div class="d-flex">
      <img src="https://images.unsplash.com/photo-1502743780242-f10d2ce370f3?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1916&q=80" alt="" class="me-2" style="width: 48px; height: 48px; object-fit: cover" />
      <div class="w-100">
        <div class="d-flex justify-content-between">
          <p class="mb-0 fw-bold">Lorem ipsum</p>
          <p class="mb-0">NT$12,000</p>
        </div>
        <p class="mb-0 fw-bold">x1</p>
      </div>
    </div> -->
    <div class="d-flex mt-2" v-for="productItem in allCartInfo.carts" :key="productItem.product.id">
      <img :src="productItem.product.imageUrl" alt="圖片" class="me-2" style="width: 48px; height: 48px; object-fit: cover" />
      <div class="w-100">
        <div class="d-flex justify-content-between">
          <p class="mb-0 fw-bold">{{ productItem.product.title }}</p>
          <p class="mb-0">NT${{ productItem.total }}</p>
        </div>
        <p class="mb-0 fw-bold">x{{ productItem.qty }}</p>
      </div>
    </div>
    <table class="table mt-4 border-top border-bottom text-muted">
      <tbody>
        <tr>
          <th scope="row" class="border-0 px-0 pt-4 font-weight-normal">Subtotal</th>
          <td class="text-end border-0 px-0 pt-4">NT${{ allCartInfo.total }}</td>
        </tr>
        <tr>
          <th scope="row" class="border-0 px-0 pt-0 pb-4 font-weight-normal">Payment</th>
          <td class="text-end border-0 px-0 pt-0 pb-4">ApplePay</td>
        </tr>
      </tbody>
    </table>
    <div class="d-flex justify-content-between mt-4">
      <p class="mb-0 h4 fw-bold">Total</p>
      <p class="mb-0 h4 fw-bold">NT${{ allCartInfo.total }}</p>
    </div>
  </div>
</template>
<style scoped></style>
